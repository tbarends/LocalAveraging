...
def writexplormap(mapoutname,totalmap,a,b,c,alpha,beta,gamma,na,mina,maxa,nb,minb,maxb,nc,minc,maxc):
    ...
    mapout.close()
##################################################
for root in ['20','35','60','80','100']:
    
    hklinname    = 'D:/PUMPPROBE/FOFO_SAMENUMBERIMAGES/LYSO/HKL/lyso_'+root+'fs.hkl'                 # text file with h k l F phi in P1
    pdbinname    = 'D:/PUMPPROBE/PUMPROBE_7.1-coot-0_refmac1.pdb'   # input coordinate file
    mapoutname   = 'D:/PUMPPROBE/FOFO_SAMENUMBERIMAGES/LYSO/lyso_'+root+'fs.xplor'               # output map file
    pdboutname   = 'D:/PUMPPROBE/FOFO_SAMENUMBERIMAGES/LYSO/lyso.pdb'
    pseoutname   = 'D:/PUMPPROBE/FOFO_SAMENUMBERIMAGES/LYSO/lyso_'+root+'fs.pse'
    a            = 79.10                                     # unit cell parameters
    b            = 79.10 
    c            = 38.50
    alpha        = 90.0
    beta         = 90.0
    gamma        = 90.0
    radius       = 6                                          # half the (box length - 1) in grid points
    step         = 0.75                                        # grid spacing in Angstrom
    chain_search = 'A'                                       # the chain identifier
    RESs         = range(1,127)                                      # a python list of residue numbers
    ...
    for resid_search in RESs:
        ...
        for n in range(len(name)):
            ...
            if resid[n]==resid_search+1 and chain_search==chain[n]:
                if name[n]=='N  ':
                    Cphe[0,0]=C[n,0]
                    Cphe[0,1]=C[n,1]
                    Cphe[0,2]=C[n,2]
                
        print '\rI found the atoms of ',chain_search,resid_search,'.'
        vector1=np.asarray([ Cphe[0,0]-Cphe[1,0] , Cphe[0,1]-Cphe[1,1] , Cphe[0,2]-Cphe[1,2]])
        xvec=vector1/np.linalg.norm(vector1)
        
        vector2=np.asarray([ Cphe[3,0]-Cphe[2,0] , Cphe[3,1]-Cphe[2,1] , Cphe[3,2]-Cphe[2,2]])
        
        yvec=vector2/np.linalg.norm(vector2)
        zvec=np.cross(xvec,yvec)
        #print 'My x vector (from Ca to N+1) will be',xvec
        #print 'My y vector (from C to O) will be',yvec
        #print 'My z vector (cross product of x and y) will be',zvec
        xmid=np.average(Cphe[:,0])
        ymid=np.average(Cphe[:,1])
        zmid=np.average(Cphe[:,2])
        mid=np.asarray([xmid,ymid,zmid])
        #print 'The center of the peptide is at',mid
    ...
    names=['ATOM      1  N   ALA A   2','ATOM      2  CA  ALA A   1','ATOM      3  C   ALA A   1','ATOM      4  O   ALA A   1']

    pdbout=open(pdboutname,'w')
    origin=np.asarray([xmid,ymid,zmid])
    for n in range(4):
        atom=np.asarray([Cphe[n,0],Cphe[n,1],Cphe[n,2]])
        atom=atom-origin
