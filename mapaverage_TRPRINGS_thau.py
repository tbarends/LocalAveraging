...
#for root in ['40']:
    ...
    mapoutname   = 'D:/PUMPPROBE/FOFO_SAMENUMBERIMAGES/THAU/thau_trp_'+root+'fs.xplor'               # output map file
    pdboutname   = 'D:/PUMPPROBE/FOFO_SAMENUMBERIMAGES/THAU/thau_trp.pdb'
    pseoutname   = 'D:/PUMPPROBE/FOFO_SAMENUMBERIMAGES/THAU/thau_trp_'+root+'fs.pse'
    ...
    beta         = 90.0
    gamma        = 90.0
    radius       = 5                                          # half the (box length - 1) in grid points
    step         = 0.75                                        # grid spacing in Angstrom
    shave        = 3          ###NEW                               # shave off n grid planes above and below ring...

    chain_search = 'A'                                       # the chain identifier

#    PHEs=[3,58,80,90,108,114,152,172,173,181,203]
#    PHEs=[11, 57, 95, 99, 157, 168, 183, 199 ] # TYROSINES!!!
#    PHEs=[14,37,51 ] # TRYPTOPHANS!!!
    PHEs=[14,37,51] # TRYPTOPHANS!!!
    ...
    print 'Map before shaving', np.shape(totalmap) ###NEW
    totalmap=totalmap[:,:,shave:(2*radius+1-shave)] ###NEW          
    print 'Map before shaving', np.shape(totalmap) ###NEW
    ...
    writexplormap(mapoutname,totalmap,mapsize,mapsize,mapsize-(2*shave*step),90.0,90.0,90.0,na,mina,maxa,nb,minb,maxb,nc,minc,maxc)
...
##        ss="          "
##        es=" "
##        outstring='ATOM    438  C   TYR A   1'+xs+ys+zs+os+bs+ss+es+'\n'
